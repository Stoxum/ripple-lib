...
function createHTTPServer(options, httpPort) {
  const stoxumAPI = new StoxumAPI(options)

  const methodNames = _.filter(_.keys(StoxumAPI.prototype), k => {
    return typeof StoxumAPI.prototype[k] === 'function'
    && k !== 'connect'
    && k !== 'disconnect'
    && k !== 'constructor'
    && k !== 'StoxumAPI'
  ...
  function applyPromiseWithCallback(fnName, callback, args_) {
    try {
      let args = args_
      if (!_.isArray(args_)) {
        const fnParameters = jayson.Utils.getParameterNames(stoxumAPI[fnName])
      ...
      Promise.resolve(stoxumAPI[fnName](...args))
